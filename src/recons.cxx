...
int main(int argc, char* argv[])
{
  ...
  if (recons_type==0) {
    ...
    std::cout << "Writting plane wave recons results to file " << file_out.str() << std::endl;
    fBox.fitModel()  = PLAN_WAVE;
  }
  else if (recons_type==1) {
    file_out << text_dir << "/Rec_sphere_wave_recons.txt";
    std::cout << "Writting Spherical wave recons results to file " << file_out.str() << std::endl;
    fBox.fitModel()  = SPHERE_WAVE;
  }
  else if (recons_type==2) {
    file_out << text_dir << "/Rec_adf_recons.txt";
    std::cout << "Writting Angular Distribution Function recons results to file " << file_out.str() << std::endl;
    fBox.fitModel()  = ADF;
  }
  else {
  ...
  int j = 0;   // Has to be initialized at 0 othetwise wrong for first coinc
  int k =1;
  int l =1;
  long int dummy_int;
  double dummy;
  ...
  double GroundAltitude = 1205.;//1086.;//2900.;
  ...
  while ( fscanf( fid_in, "%ld %ld %lf %lf",&Id_antenna,&iCoinc,&trig,&amp)>0 )  {  // New coinctable.txt format
      ...
      if (iCoinc_prev != iCoinc || i==0 )  {  // New coinc
        ...
        fBox.Na() = j;
        //std::cout << "Multiplicity = " << fBox.Na() << std::endl;
        if (fBox.Na()>3){
	    ...
	    std::cout<< "theta = "<<fBox.theta()*RAD2DEG<<" phi = "<<fBox.phi()*RAD2DEG<<std::endl;
          }
          else if (recons_type==1){ // Spherical wave
              fprintf( fid_out, "%ld %3.0d %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le \n", iCoinc_prev, fBox.Na(), chi2, fBox.chi2_significance(), - fBox.r_xmax()*(sin(fBox.theta())*cos(fBox.phi())), - fBox.r_xmax()*(sin(fBox.theta())*sin(fBox.phi())),  - fBox.r_xmax()*cos(fBox.theta()), fBox.ts(), fBox.r_xmax(), GroundAltitude); // Physical spherical wave parameters
              std::cout<<"rho = "<< fBox.r_xmax()<<" theta = "<<fBox.theta()*RAD2DEG<<" phi = "<<fBox.phi()*RAD2DEG<<std::endl;
        ...
        j=1;  // Has to be initialized at 1 to account for first unit
        k=0;
        iCoinc_prev = iCoinc;
        l++;
      }
      else  {   // Same coinc
        //std::cout << j << " " << amp << std::endl;
	j++;  
      }
  ...
  std::cout << "Writing last recons..." << std::endl;
  fBox.Na() = j;
  if ( fBox.Na()>3  && (fBox.ta(2)+fBox.ta(3)>0 ) ) {  // Timing performed for intercorrelation
    ...
    if (recons_type==0)  {   // Plan wave
      ...
      fprintf( fid_out, "%ld %3.0d %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le  %12.5le %12.5le\n", iCoinc_prev, fBox.Na(), chi2, fBox.chi2_significance(), - fBox.r_xmax()*(sin(fBox.theta())*cos(fBox.phi())), - fBox.r_xmax()*(sin(fBox.theta())*sin(fBox.phi())), - fBox.r_xmax()*cos(fBox.theta()), fBox.ts(), fBox.r_xmax(), GroundAltitude); // Physical Hyperbolic wave parameters
    }
    else if (recons_type==2){ //ADF
        fprintf( fid_out, "%ld %3.0d %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le %12.5le\n", iCoinc_prev, fBox.Na(), fBox.theta()*RAD2DEG, fBox.theta_error()*RAD2DEG, fBox.phi()*RAD2DEG, fBox.phi_error()*RAD2DEG, chi2,fBox.chi2_significance(), fBox.cerenkov_width(), fBox.amp_corr()); // ADF parameters
      }
